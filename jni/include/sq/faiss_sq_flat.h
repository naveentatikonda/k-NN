    ...
    struct FaissSQDistanceComputer final : faiss::DistanceComputer {
        ...
        FaissSQDistanceComputer(int32_t _oneElementByteSize, const void* _data, float _centroidDp, int32_t _dimension, int32_t _numVectors, int32_t _docBits)
          : faiss::DistanceComputer(),
            ...
            // Validate docBits early. Without this, a bad value from the caller would:
            //   - docBits == 0    -> planeBytes computes as `qvb / 0` (SIGFPE / UB)
            //   - docBits > 30    -> `1 << _docBits` is signed shift UB before we cast to float
            //   - docBits ∉ {1,2,4} -> silent wrong reconstruction / graph corruption
            // Reject at construction so the failure is loud at build time, not at query time.
            if (!isMosDocBits(_docBits)) {
                throw std::runtime_error(
                    "FaissSQDistanceComputer: unsupported docBits=" + std::to_string(_docBits)
                    + ". Supported: 1, 2, 4."
                );
            }
            // Layout requirements differ per docBits:
            //   B=1: one bit-plane, qvb = ceil(dim/8)              (any positive value)
            //   B=2: two bit-planes, qvb = 2 * ceil(dim/8)         (must be a multiple of 2)
            //   B=4: PACKED_NIBBLE, 2 elements per byte, qvb = ceil(dim/2)  (any positive value)
            // The bit-plane kernel for B=2 requires equal-length planes, so we keep the
            // divisibility check there. B=4 uses Lucene's packed-nibble bytes verbatim, so
            // the byte count only needs to be positive (which oneElementByteSize > 16 guarantees).
            if (_docBits == 2 && quantizedVectorBytes % 2 != 0) {
                throw std::runtime_error(
                    "FaissSQDistanceComputer: quantizedVectorBytes=" + std::to_string(quantizedVectorBytes)
