...
public class MemOptimizedScalarQuantizedIndexBuildStrategy implements NativeIndexBuildStrategy {
    ...
    public void buildAndWriteIndex(final BuildIndexParams indexInfo) throws IOException, IndexBuildAbortedException {
        ...
        // Document bit width (1, 2, or 4), derived from the stored encoding. Passed to the
        // native side so it can dispatch the correct distance kernel: bit-plane popcount
        // for B=1/2, byte-wise nibble multiply for B=4.
    ...
    private void passQuantizedVectorsAndCorrectionFactors(
        ...
        for (int i = 0; i < binarizedVectorValues.size();) {
            ...
            for (int j = 0, o = 0; j < loopSize; ++j) {
                ...
                // Lucene layout directly, so all three widths flow through with a single arraycopy.
                ...
                // Copy the quantized code into the buffer verbatim. The native side handles
                // the per-width layout (bit-plane for B=1/2, packed-nibble for B=4) directly.
